...
def signin(request):
    ...
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            messages.info(request, 'Credentials Invalid, Email or Password is incorrect')
...
def SmartInvigilationProject(request):
    import os
...
# CODES ZA KUGET USERNAME AND PASSWORD
    ...
    if request.method == 'POST':
        ...
        camera_no = request.POST.get('camera_no')
        to_phone_number = request.user.phone
        #from_phone_number = request.POST.get('from_phone_number')

        save_invigilator = InvigilationStaffs.objects.create(username=username, camera_no=camera_no, email=email, to_phone_number=to_phone_number)
        save_invigilator.save()
        ...
        def predFacePose(frame):
            ...
            if bbox_ is not None and prob_ is not None and landmarks_ is not None:
                            ...
                            if ((int(angR) in range(35, 57)) and (int(angL) in range(35, 58))):
                                ...
                                if angR < angL:
                                    ...
                                    if angL > 80: #-80
                                        ...
                                        #HIZI NI KWA AJILI KUTUMA EMAIL KWA INVIGILATOR
                                        
                                        
                                        # subject = "SMART INVIGILATION SYSTEM"
                                        # message = f"Hey {username} Some of the students are cheating Right, Please! Check your screen"
                                        # from_email = settings.EMAIL_HOST_USER
                                        # recipient_list = [email]
                                        # send_mail(subject, message, from_email, recipient_list, fail_silently=True)

                                        #MWISHO WA KWA AJILI KUTUMA EMAIL KWA INVIGILATOR


                                        #HIZI NI KWA AJILI YA KUTUMA SMS KWA INVIGILATOOR
                                        
                                        

                                        # account_sid = "ACec294a515a22883f25914c532734bf23"
                                        # auth_token = "0d924ac3503fe120262b106a4c7a0bd3"
                                        # client = Client(account_sid, auth_token)
                                        # message = client.messages \
                                        #             .create(
                                        #                 body="Some of the students are cheating Right, Please! Check your screen",
                                        #                 from_='+14068023763',
                                        #                 #to='+255628431507'
                                        #                 to =to_phone_number
                                        #                         )
                                        # print("Some of the students are cheating Right, Please! Check your screen")
                                        
                                        

                                        #MWISHO WA KUTUMA MSG KWA INVIGILATOR
                                    ...
                                    if angR > 80: #80
                                        ...
                                        # subject = "SMART INVIGILATION SYSTEM"
                                        # message = f"Hey {username} Some of the students are cheating Left, Please! Check your screen"
                                        # from_email = settings.EMAIL_HOST_USER
                                        # recipient_list = [email]
                                        # send_mail(subject, message, from_email, recipient_list, fail_silently=True)

                                        #MWISHO KWA AJILI KUTUMA EMAIL KWA INVIGILATOR
                                        ...
                                        # message = client.messages \
                                        #             .create(
                                        #                 body="Some of the students are cheating Left, Please! Check your screen",
                                        #                 from_='+14068023763',
                                        #                 #to='+255628431507'
                                        #                 to =to_phone_number
                                        #                         )
                                        # print("Some of the students are cheating Left, Please! Check your screen")

                                        #MWISHO KWA AJILI YA KUTUMA SMS KWA INVIGILATOOR
                ...
                #return landmarks_, angle_R_List, angle_L_List, predLabelList
                print("There is no face to detect")
                # print("None")
        ...
        def predFacePoseApp():
            ...
            out=cv2.VideoWriter(BASE_DIR+f'/saved-media/SmartInvigilationVideo-{random_number}.avi',fourcc,10,(frame_width,frame_height))
            ...
            #MWANZO WA KUCAPTURE IMAGES

            
            load = cv2.CascadeClassifier(BASE_DIR+'/cascades/data/haarcascade_frontalface_default.xml')
            ...
            while True:
                ...
                #     val=val+1
                #     cv2.imwrite(BASE_DIR+"/CheatingStudentsCapturedImages/"+str(random_number)+"."+str(val)+".jpg",gray[y:y+h,x:x+w])
                #     #cv2.imwrite(BASE_DIR+"/Data/"+str(val)+".jpg",gray[y:y+h,x:x+w])
                ...
                if not has_frame:
                    #break
                    print("It is not possible to read image from the camera")
                    continue
                ...
                # print(angle_R_List)
                # print(angle_L_List)
                # angle_L_List and angle_R_List are list inwhich
                # the left and right angles are appended
                ...
                for Left_angle in angle_R_List:
                    if Left_angle > 80:
                        #MWANZO WA KUCAPTURE IMAGES
                        
                        gray2 = cv2.cvtColor(frame,cv2.COLOR_BGR2GRAY)
                        faces2 = load.detectMultiScale(gray2,1.3,5)
                        
                        
                        for (x,y,w,h) in faces2:
                            val=val+1
                            cv2.imwrite(BASE_DIR+"/LeftCapturedImages/"+str(Left_angle)+"."+str(val)+".jpg",gray2[y:y+h,x:x+w])
                ...
                for Right_angle in angle_L_List:
                    if Right_angle > 80:
                        #MWANZO WA KUCAPTURE IMAGES
                        ...
                        for (x,y,w,h) in faces2:
                            val=val+1
                            cv2.imwrite(BASE_DIR+"/RightCapturedImages/"+str(Left_angle)+"."+str(val)+".jpg",gray2[y:y+h,x:x+w])
...
def wifi_page(request):
    ...
    return render(request, 'SmartInvigilationApp/wifi_page.html')
...
def record_video(request):
    ...
    if request.method == 'POST':
        ...
        save_invigilator = InvigilationStaffs.objects.create(username=username, camera_no=camera_no)
        save_invigilator.save()
        ...
        # Create a video capture object from the VideoCapture Class.
        #video_cap = cv2.VideoCapture(BASE_DIR+"/videos/4.mp4")
        mycamera = cv2.VideoCapture(BASE_DIR+mysource)
        ...
        #km unaingiza no path itakuwa
        #mycamera = cv2.VideoCapture(mysource)
        ...
        return redirect('recording_video_page')

    messages.success(request, f"Error! Form is not valid")
    return redirect('recording_video_page')

    #return render(request, 'SmartInvigilationApp/recording_video_page.html')



@login_required(login_url='homePage')
def recording_video_page(request):
